...
func Test_sshConfigSplitOnWirtualSection(t *testing.T) {
	t.Parallel()
	...
	for _, tc := range testCases {
		...
		t.Run(tc.Name, func(t *testing.T) {
			...
			before, section, after, err := sshConfigSplitOnWirtualSection([]byte(tc.Input))
			if tc.Err {
				require.Error(t, err)
				return
			}
...
func Test_sshConfigExecEscape(t *testing.T) {
	...
	tests := []struct {
		...
		{"windows path", `C:\Program Files\Wirtual\bin\wirtual.exe`, false},
...
func Test_sshConfigExecEscapeSeparatorForce(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			path:           `C:\Program Files\Wirtual\bin\wirtual.exe`,
			expWindowsPath: `"C:\Program Files\Wirtual\bin\wirtual.exe"`,
			expOtherPath:   `"C:\Program Files\Wirtual\bin\wirtual.exe"`,
			forceUnix:      false,
			wantErr:        false,
		},
		{
			name:           "windows_keep_forward_slashes",
			path:           `C:\ProgramFiles\Wirtual\bin\wirtual.exe`,
			expWindowsPath: `C:\ProgramFiles\Wirtual\bin\wirtual.exe`,
			expOtherPath:   `C:\ProgramFiles\Wirtual\bin\wirtual.exe`,
			forceUnix:      false,
			wantErr:        false,
		},
		{
