...
func TestConfigSSH(t *testing.T) {
	...
	if runtime.GOOS == "windows" {
		t.Skip("See wirtual/internal#117")
	}

	const hostname = "test-wirtual."
	...
	pty = ptytest.New(t)
	// Set HOME because wirtual config is included from ~/.ssh/wirtual.
...
func TestConfigSSH_FileWriteAndOptionsFlow(t *testing.T) {
	...
	headerStart := strings.Join([]string{
		"# ------------START-WIRTUAL-----------",
		"# This section is managed by wirtual. DO NOT EDIT.",
		"#",
	...
	tests := []struct {
		...
		},
		{
			...
			writeConfig: writeConfig{
				ssh: strings.Join([]string{
					"# Content before wirtual block",
					headerEnd,
					headerStart,
					"# Content after wirtual block",
				}, "\n"),
			},
			wantErr: true,
		},
		...
		},
		{
			...
			args: []string{
				"-y", "--wirtual-binary-path", "/foo/bar/wirtual",
			},
			wantErr:  false,
			hasAgent: true,
			wantConfig: wantConfig{
				regexMatch: "ProxyCommand /foo/bar/wirtual",
			},
		},
		{
			name: "Header",
...
func TestConfigSSH_Hostnames(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			expected: []string{"wirtual.@", "wirtual.@.agent1"},
		},
		{
			...
			expected: []string{"wirtual.@.agent1", "wirtual.@.agent2"},
		},
		{
