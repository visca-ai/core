...
func TestLogout(t *testing.T) {
	...
	t.Run("Logout", func(t *testing.T) {
		...
		pty.WriteLine("yes")
		pty.ExpectMatch("You are no longer logged in. You can log in using 'wirtual login <url>'.")
		<-logoutChan
	})
	...
	t.Run("NoURLFile", func(t *testing.T) {
		...
		go func() {
			defer close(logoutChan)
			err := logout.Run()
			assert.ErrorContains(t, err, "You are not logged in. Try logging in using 'wirtual login <url>'.")
		}()
