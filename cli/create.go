...
func (r *RootCmd) create() *serpent.Command {
	...
	cmd := &serpent.Command{
		...
		Long: FormatExamples(
			Example{
				...
				Command:     "wirtual create <username>/<workspace_name>",
			},
		),
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			if copyParametersFrom != "" {
				...
				_, _ = fmt.Fprintf(inv.Stdout, "Wirtual will use the same template %q as the source workspace.\n", sourceWorkspace.TemplateName)
	...
	cmd.Options = append(cmd.Options,
		...
		serpent.Option{
			...
			Description: "Specify the workspace autostart schedule. Check wirtual schedule start --help for the syntax.",
