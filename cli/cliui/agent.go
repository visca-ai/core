...
func PeerDiagnostics(w io.Writer, d tailnet.PeerDiagnostics) {
	...
	if d.SentNode {
		_, _ = fmt.Fprint(w, "✔ sent local data to Wirtual networking coordinator\n")
	} else {
		_, _ = fmt.Fprint(w, "✘ have not sent local data to Wirtual networking coordinator\n")
	}
	if d.ReceivedNode != nil {
		...
		_, _ = fmt.Fprintf(w,
			"✔ received remote agent data from Wirtual networking coordinator\n    preferred DERP region: %s %s\n    endpoints: %s\n",
			dp, dn, strings.Join(d.ReceivedNode.Endpoints, ", "))
	} else {
		_, _ = fmt.Fprint(w, "✘ have not received remote agent data from Wirtual networking coordinator\n")
	}
...
func (d ConnDiags) splitDiagnostics() (general, client, agent []string) {
	...
	if d.ConnInfo.DisableDirectConnections {
		general = append(general,
			fmt.Sprintf("❗ Your Wirtual administrator has blocked direct connections\n   %s#disabled-deployment-wide", d.TroubleshootingURL))
		if !d.Verbose {
			return general, client, agent
		}
	}
