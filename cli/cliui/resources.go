...
// │ └─ dev (linux, amd64)        ⦾ connecting [10s]    wirtual ssh dev.dev       │
// ├────────────────────────────────────────────────────────────────────────────┤
// │ kubernetes_pod.dev                                                         │
// │ ├─ go (linux, amd64)         ⦿ connected           wirtual ssh dev.go        │
// │ └─ postgres (linux, amd64)   ⦾ disconnected [4s]   wirtual ssh dev.postgres  │
// └────────────────────────────────────────────────────────────────────────────┘
func WorkspaceResources(writer io.Writer, resources []wirtualsdk.WorkspaceResource, options WorkspaceResourcesOptions) error {
	...
	for _, resource := range resources {
		...
		for index, agent := range resource.Agents {
			...
			if !options.HideAccess {
				sshCommand := "wirtual ssh " + options.WorkspaceName
				if totalAgents > 1 {
