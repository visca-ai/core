...
import (
	...
	"github.com/wirtualdev/wirtualdev/v2/wirtuald/healthcheck/health"
...
func TestPeerDiagnostics(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			want: []*regexp.Regexp{
				regexp.MustCompile(`^✔ sent local data to Wirtual networking coordinator$`),
			},
		},
		{
			name: "didntSendNode",
			...
			want: []*regexp.Regexp{
				regexp.MustCompile(`^✘ have not sent local data to Wirtual networking coordinator$`),
			},
		},
		{
			...
			want: []*regexp.Regexp{
				regexp.MustCompile(`^✔ received remote agent data from Wirtual networking coordinator$`),
		...
		},
		{
			...
			want: []*regexp.Regexp{
				regexp.MustCompile(`^✘ have not received remote agent data from Wirtual networking coordinator$`),
			},
		},
		{
...
func TestConnDiagnostics(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			want: []string{
				`❗ Your Wirtual administrator has blocked direct connections`,
			},
		},
		{
			name: "NoStun",
