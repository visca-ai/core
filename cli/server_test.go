...
import (
	...
	"github.com/wirtualdev/wirtualdev/v2/tailnet/tailnettest"
...
func TestServer(t *testing.T) {
	...
	t.Run("OIDC", func(t *testing.T) {
		...
		t.Run("Defaults", func(t *testing.T) {
			...
			// This is just needed to get Wirtual to start up.
		...
		t.Run("Overrides", func(t *testing.T) {
			...
			inv, cfg := clitest.New(t,
				...
				"--oidc-sign-in-text", "Sign In With Wirtual",
			...
			require.Equal(t, "Sign In With Wirtual", deploymentConfig.Values.OIDC.SignInText.Value())
			...
			for _, opt := range deploymentConfig.Options {
				...
				case "oidc-sign-in-text":
					require.Equal(t, "Sign In With Wirtual", opt.Value.String())
	...
	t.Run("Logging", func(t *testing.T) {
		...
		t.Run("CreatesFile", func(t *testing.T) {
			t.Parallel()
			fiName := testutil.TempFile(t, "", "wirtual-logging-test-*")
		...
		t.Run("Human", func(t *testing.T) {
			t.Parallel()
			fi := testutil.TempFile(t, "", "wirtual-logging-test-*")
		...
		t.Run("Multiple", func(t *testing.T) {
			...
			fi1 := testutil.TempFile(t, "", "wirtualal-logging-test-*")
			fi2 := testutil.TempFile(t, "", "wirtual-logging-test-*")
			fi3 := testutil.TempFile(t, "", "wirtual-logging-test-*")
	...
	t.Run("YAML", func(t *testing.T) {
		...
		t.Run("WriteThenReadConfig", func(t *testing.T) {
			...
			args := []string{
				...
				"--http-address", ":0",
				"--access-url", "http://example.com",
				"--log-human", filepath.Join(t.TempDir(), "wirtual-logging-test-human"),
			...
			fi, err := os.OpenFile(testutil.TempFile(t, "", "wirtual-config-test-*"), os.O_WRONLY|os.O_CREATE, 0o600)
			require.NoError(t, err)
			defer fi.Close()
...
func TestServer_InterruptShutdown(t *testing.T) {
	...
	require.NoError(t, err)
	// We cannot send more signals here, because it's possible Wirtual
