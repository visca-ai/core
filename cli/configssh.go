...
const (
	...
	sshConfigSectionHeader   = "# This section is managed by wirtual. DO NOT EDIT."
	sshConfigDocsHeader      = `
#
# You should not hand-edit this section unless you are removing it, all
# changes will be lost when running "wirtual config-ssh".
`
...
// from the wirtual config in ~/.ssh/wirtual.
...
func (r *RootCmd) configSSH() *serpent.Command {
	var (
		...
		wirtualCliPath      string
	)
	client := new(wirtualsdk.Client)
	cmd := &serpent.Command{
		Annotations: workspaceCommand,
		Use:         "config-ssh",
		Short:       "Add an SSH Host entry for your workspaces \"ssh wirtual.workspace\"",
		Long: FormatExamples(
			Example{
				Description: "You can use -o (or --ssh-option) so set SSH options to be used for all your workspaces",
				Command:     "wirtual config-ssh -o ForwardAgent=yes",
			},
			Example{
				Description: "You can use --dry-run (or -n) to see the changes that would be made",
				Command:     "wirtual config-ssh --dry-run",
			},
		),
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			var err error
			wirtualBinary := wirtualCliPath
			if wirtualBinary == "" {
				wirtualBinary, err = currentBinPath(out)
				if err != nil {
					return err
				}
			}

			escapedWirtualBinary, err := sshConfigExecEscape(wirtualBinary, forceUnixSeparators)
			if err != nil {
				return xerrors.Errorf("escape wirtual binary for ssh failed: %w", err)
			}
			...
			section, ok, err := sshConfigGetWirtualSection(configRaw)
			if err != nil {
				return err
			}
			if ok {
			...
			buf := &bytes.Buffer{}
			before, _, after, err := sshConfigSplitOnWirtualSection(configModified)
			if err != nil {
				return err
			}
			...
			if err != nil {
				...
				wirtualdConfig.HostnamePrefix = "wirtual."
			}
				...
				for _, workspaceHostname := range wc.Hosts {
					...
					if !skipProxyCommand {
						...
						defaultOptions = append(defaultOptions, fmt.Sprintf(
							...
							escapedWirtualBinary, rootFlags, flags, workspaceHostname,
						))
					}
			...
			if !bytes.Equal(configModified, buf.Bytes()) {
				changes = append(changes, fmt.Sprintf("Update the wirtual section in %s", sshConfigFile))
	...
	cmd.Options = serpent.OptionSet{
		...
		{
			Flag:    "wirtual-binary-path",
			Env:     "WIRTUAL_SSH_CONFIG_BINARY_PATH",
			Default: "",
			Description: "Optionally specify the absolute path to the wirtual binary used in ProxyCommand. " +
				"By default, the binary invoking this command ('config ssh') is used.",
			Value: serpent.Validate(serpent.StringOf(&wirtualCliPath), func(value *serpent.String) error {
				...
				if !absolute {
					return xerrors.Errorf("wirtual cli path must be an absolute path")
				}
				return nil
			}),
		},
...
// sshConfigGetWirtualSection is a helper function that only returns the wirtual
// section of the SSH config and a boolean if it exists.
func sshConfigGetWirtualSection(data []byte) (section []byte, ok bool, err error) {
	_, section, _, err = sshConfigSplitOnWirtualSection(data)
	if err != nil {
...
// sshConfigSplitOnWirtualSection splits the SSH config into 3 sections.
// All lines before sshStartToken, the wirtual section, and all lines after
// sshEndToken.
func sshConfigSplitOnWirtualSection(data []byte) (before, section []byte, after []byte, err error) {
	startCount := bytes.Count(data, []byte(sshStartToken))
	endCount := bytes.Count(data, []byte(sshEndToken))
	if startCount > 1 || endCount > 1 {
		return nil, nil, nil, xerrors.New("Malformed config: ssh config has multiple wirtual sections, please remove all but one")
	}
	...
	if startIndex != -1 && endIndex != -1 {
		if startIndex > endIndex {
			return nil, nil, nil, xerrors.New("Malformed config: ssh config has wirtual section, but it is malformed and the END header is before the START header")
		}
...
//
//	ProxyCommand "/path/with space/wirtual" ssh --stdio work
//
// This is ~what OpenSSH would execute:
//
//	/bin/bash -c '"/path/with space/to/wirtual" ssh --stdio workspace'
//
...
//
// See:
//   - https://github.com/wirtualdev/wirtualdev/issues/2853
...
// git bash. In these situations the wirtual.exe is using the windows filepath
...
// currentBinPath returns the path to the wirtual binary suitable for use in ssh
// ProxyCommand.
func currentBinPath(w io.Writer) (string, error) {
	...
	// On Windows, the wirtual-cli executable must be in $PATH for both Msys2/Git
