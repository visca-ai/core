...
func (r *RootCmd) tokens() *serpent.Command {
	cmd := &serpent.Command{
		...
		Long: "Tokens are used to authenticate automated clients to Wirtual.\n" + FormatExamples(
			Example{
				Description: "Create a token for automation",
				Command:     "wirtual tokens create",
			},
			Example{
				Description: "List your tokens",
				Command:     "wirtual tokens ls",
			},
			Example{
				Description: "Remove a token by ID",
				Command:     "wirtual tokens rm WuoWs4ZsMX",
			},
		),
