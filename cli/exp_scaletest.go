...
import (
	...
	"github.com/wirtualdev/wirtualdev/v2/scaletest/agentconn"
	"github.com/wirtualdev/wirtualdev/v2/scaletest/createworkspaces"
	"github.com/wirtualdev/wirtualdev/v2/scaletest/dashboard"
	"github.com/wirtualdev/wirtualdev/v2/scaletest/harness"
	"github.com/wirtualdev/wirtualdev/v2/scaletest/reconnectingpty"
	"github.com/wirtualdev/wirtualdev/v2/scaletest/workspacebuild"
	"github.com/wirtualdev/wirtualdev/v2/scaletest/workspacetraffic"
	...
	"github.com/wirtualdev/wirtualdev/v2/wirtualsdk/workspacesdk"
)

const scaletestTracerName = "wirtual_scaletest"
	...
	*opts = append(
		*opts,
		...
		serpent.Option{
			...
			Description: "Whether opentelemetry traces are sent to Wirtual. We recommend keeping this disabled unless we advise you to enable it.",
			Value:       serpent.BoolOf(&s.traceWirtual),
		},
		serpent.Option{
		...
		serpent.Option{
			...
			Description: "Enables trace propagation to the Wirtual backend, which will be used to correlate server-side spans with client-side spans. Only enable this if the server is configured with the exact same tracing configuration as the client.",
...
func (s *scaletestTracingFlags) provider(ctx context.Context) (trace.TracerProvider, func(context.Context) error, bool, error) {
	shouldTrace := s.traceEnable || s.traceWirtual || s.traceHoneycombAPIKey != ""
	if !shouldTrace {
...
func (r *RootCmd) scaletestCreateWorkspaces() *serpent.Command {
	...
	cmd.Options = serpent.OptionSet{
		...
		},
		{
			...
			Description: "Do not clean up resources after the test completes. You can cleanup manually using wirtual scaletest cleanup.",
