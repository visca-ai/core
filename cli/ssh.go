...
import (
	...
	"github.com/wirtualdev/wirtualdev/v2/pty"
	"github.com/wirtualdev/wirtualdev/v2/wirtuald/autobuild/notify"
...
func (r *RootCmd) ssh() *serpent.Command {
	...
	cmd := &serpent.Command{
		...
		Handler: func(inv *serpent.Invocation) (retErr error) {
			...
			// See: https://github.com/wirtualdev/wirtualdev/issues/13144
...
//
// See: https://github.com/wirtualdev/wirtualdev/issues/6180
...
startWatchLoop:
	for {
		logger.Debug(ctx, "connecting to the wirtual server to watch workspace events")
...
func tryPollWorkspaceAutostop(ctx context.Context, client *wirtualsdk.Client, workspace wirtualsdk.Workspace) (stop func()) {
	lock := flock.New(filepath.Join(os.TempDir(), "wirtual-autostop-notify-"+workspace.ID.String()))
	...
	return func(now time.Time) (deadline time.Time, callback func()) {
		...
		callback = func() {
			...
			if ttl > time.Minute {
				...
				body = fmt.Sprintf(
					`Your Wirtual workspace %s is scheduled to stop in %.0f mins`, ws.Name, ttl.Minutes())
			} else {
				title = fmt.Sprintf("Workspace %s stopping!", ws.Name)
				body = fmt.Sprintf("Your Wirtual workspace %s is stopping any time now!", ws.Name)
			}
...
// Build the user workspace link which navigates to the Wirtual web UI.
