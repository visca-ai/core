...
const (
	...
	notLoggedInMessage = "You are not logged in. Try logging in using 'wirtual login <url>'."
...
func (r *RootCmd) Command(subcommands []*serpent.Command) (*serpent.Command, error) {
	fmtLong := `Wirtual %s — Training Ground For Next Billion Physical AI Agents.
`
	cmd := &serpent.Command{
		Use: "wirtual [global-flags] <subcommand>",
		Long: fmt.Sprintf(fmtLong, buildinfo.Version()) + FormatExamples(
			Example{
				Description: "Get started by creating a template from an example",
				Command:     "wirtual templates init",
			},
		),
		Handler: func(i *serpent.Invocation) error {
			...
			// cross-platform scripts to invoke the Wirtual binary
	...
	cmd.Options = serpent.OptionSet{
		...
		},
		{
			...
			Description: "Disable network telemetry. Network telemetry is collected when connecting to workspaces using the CLI, and is forwarded to the server. If telemetry is also enabled on the server, it may be sent to Wirtual. Network telemetry is used to measure network quality and detect regressions.",
		...
		},
		{
			...
			Description: "Path to the global `wirtual` config directory.",
		...
		},
		{
			...
			// They have two Wirtual CLIs, and want to tell the difference by running
...
func DumpHandler(ctx context.Context, name string) {
	...
	for {
		...
		fpath := filepath.Join(dir, fmt.Sprintf("wirtual-%s-%s.dump", name, filesystemSafeTime))
...
func SlimUnsupported(w io.Writer, cmd string) {
	_, _ = fmt.Fprintf(w, "You are using a 'slim' build of Wirtual, which does not support the %s subcommand.\n", pretty.Sprint(cliui.DefaultStyles.Code, cmd))
	_, _ = fmt.Fprintln(w, "")
	_, _ = fmt.Fprintln(w, "Please use a build of Wirtual from GitHub releases:")
	_, _ = fmt.Fprintln(w, "  https://github.com/wirtualdev/wirtualdev/releases")
...
func defaultUpgradeMessage(version string) string {
	...
	if runtime.GOOS == "windows" {
		return fmt.Sprintf("download the server version from: https://github.com/wirtualdev/wirtualdev/releases/v%s", version)
	}
	return fmt.Sprintf("download the server version with: 'curl -L https://wirtual.dev/install.sh | sh -s -- --version %s'", version)
}
