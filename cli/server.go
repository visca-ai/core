...
import (
	...
	"github.com/wirtualdev/quartz"
	"github.com/wirtualdev/retry"
	"github.com/wirtualdev/serpent"
	"github.com/wirtualdev/wgtunnel/tunnelsdk"

	"cdr.dev/slog"
	"cdr.dev/slog/sloggers/sloghuman"

	"github.com/wirtualdev/wirtualdev/v2/wirtuald/entitlements"
	"github.com/wirtualdev/wirtualdev/v2/wirtuald/notifications/reports"
	"github.com/wirtualdev/wirtualdev/v2/wirtuald/runtimeconfig"

	"github.com/wirtualdev/wirtualdev/v2/buildinfo"
	"github.com/wirtualdev/wirtualdev/v2/cli/clilog"
	...
	"github.com/wirtualdev/wirtualdev/v2/provisioner/terraform"
	"github.com/wirtualdev/wirtualdev/v2/provisionerd"
	"github.com/wirtualdev/wirtualdev/v2/provisionerd/proto"
	"github.com/wirtualdev/wirtualdev/v2/provisionersdk"
	sdkproto "github.com/wirtualdev/wirtualdev/v2/provisionersdk/proto"
	"github.com/wirtualdev/wirtualdev/v2/tailnet"
	"github.com/wirtualdev/wirtualdev/v2/wirtuald"
	"github.com/wirtualdev/wirtualdev/v2/wirtuald/autobuild"
	"github.com/wirtualdev/wirtualdev/v2/wirtuald/database"
	"github.com/wirtualdev/wirtualdev/v2/wirtuald/database/awsiamrds"
	"github.com/wirtualdev/wirtualdev/v2/wirtuald/database/dbauthz"
	"github.com/wirtualdev/wirtualdev/v2/wirtuald/database/dbmem"
	"github.com/wirtualdev/wirtualdev/v2/wirtuald/database/dbmetrics"
	"github.com/wirtualdev/wirtualdev/v2/wirtuald/database/dbpurge"
	"github.com/wirtualdev/wirtualdev/v2/wirtuald/database/migrations"
	"github.com/wirtualdev/wirtualdev/v2/wirtuald/database/pubsub"
	"github.com/wirtualdev/wirtualdev/v2/wirtuald/devtunnel"
	"github.com/wirtualdev/wirtualdev/v2/wirtuald/externalauth"
	"github.com/wirtualdev/wirtualdev/v2/wirtuald/gitsshkey"
	"github.com/wirtualdev/wirtualdev/v2/wirtuald/httpmw"
	"github.com/wirtualdev/wirtualdev/v2/wirtuald/notifications"
	"github.com/wirtualdev/wirtualdev/v2/wirtuald/oauthpki"
	"github.com/wirtualdev/wirtualdev/v2/wirtuald/prometheusmetrics"
	"github.com/wirtualdev/wirtualdev/v2/wirtuald/prometheusmetrics/insights"
	"github.com/wirtualdev/wirtualdev/v2/wirtuald/promoauth"
	"github.com/wirtualdev/wirtualdev/v2/wirtuald/schedule"
	"github.com/wirtualdev/wirtualdev/v2/wirtuald/telemetry"
	"github.com/wirtualdev/wirtualdev/v2/wirtuald/tracing"
	"github.com/wirtualdev/wirtualdev/v2/wirtuald/unhanger"
	"github.com/wirtualdev/wirtualdev/v2/wirtuald/updatecheck"
	"github.com/wirtualdev/wirtualdev/v2/wirtuald/util/slice"
	stringutil "github.com/wirtualdev/wirtualdev/v2/wirtuald/util/strings"
	"github.com/wirtualdev/wirtualdev/v2/wirtuald/workspaceapps/appurl"
	"github.com/wirtualdev/wirtualdev/v2/wirtuald/workspacestats"
	"github.com/wirtualdev/wirtualdev/v2/wirtualsdk"
	"github.com/wirtualdev/wirtualdev/v2/wirtualsdk/drpc"
)
	...
	serverCmd := &serpent.Command{
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			if isLocal || err != nil {
				...
				cliui.Warnf(
					inv.Stderr,
					"The access URL %s %s, this may cause unexpected problems when creating workspaces. Generate a unique *.app.wirtual.dev URL by not specifying an access URL.\n",
				...
				options.UpdateCheckOptions = &updatecheck.Options{
					...
					Notify: func(r updatecheck.Result) {
						if semver.Compare(r.Version, buildinfo.Version()) > 0 {
							options.Logger.Info(
								context.Background(),
								"new version of wirtual available",
			...
			if vals.OIDC.ClientKeyFile != "" || vals.OIDC.ClientSecret != "" {
				if vals.OIDC.IgnoreEmailVerified {
					logger.Warn(ctx, "wirtual will not check email_verified for OIDC logins")
				}
			...
			// We use a separate wirtualAPICloser so the Enterprise API
			// can have its own close functions. This is cleaner
			// than abstracting the Wirtual API itself.
			wirtualAPI, wirtualAPICloser, err := newAPI(ctx, options)
			if err != nil {
				return xerrors.Errorf("create wirtual API: %w", err)
			}

			if vals.Prometheus.Enable {
				// Agent metrics require reference to the tailnet coordinator, so must be initiated after Wirtual API.
				closeAgentsFunc, err := prometheusmetrics.Agents(ctx, logger, options.PrometheusRegistry, wirtualAPI.Database, &wirtualAPI.TailnetCoordinator, wirtualAPI.DERPMap, wirtualAPI.Options.AgentInactiveDisconnectTimeout, 0)
				if err != nil {
			...
			// Wirtual may be ran as users that don't have permission to write in the homedir,
			...
			if notificationsCfg.Enabled() {
				...
				cliui.Info(inv.Stdout, "Notifications are currently disabled as there are no configured delivery methods. See https://wirtual.dev/docs/admin/monitoring/notifications#delivery-methods for more details.")
			}
			...
			for i := int64(0); i < vals.Provisioner.Daemons.Value(); i++ {
				...
				daemon, err := newProvisionerDaemon(
					ctx, wirtualAPI, provisionerdMetrics, logger, vals, daemonCacheDir, errCh, &provisionerdWaitGroup, name, provisionerTypes,
				)
				if err != nil {
			...
			var handler http.Handler = wirtualAPI.RootHandler
			...
			// See: https://github.com/wirtualdev/wirtualdev/pull/3730
			//nolint:gosec
			...
			autobuildExecutor := autobuild.NewExecutor(
				ctx, options.Database, options.Pubsub, options.PrometheusRegistry, wirtualAPI.TemplateScheduleStore, &wirtualAPI.Auditor, wirtualAPI.AccessControlStore, logger, autobuildTicker.C, options.NotificationsEnqueuer)
...
func newProvisionerDaemon(
	...
	for _, provisionerType := range provisionerTypes {
		...
		case wirtualsdk.ProvisionerTypeTerraform:
			...
			tracer := wirtualAPI.TracerProvider.Tracer(tracing.TracerName)
	...
	return provisionerd.New(func(dialCtx context.Context) (proto.DRPCProvisionerDaemonClient, error) {
		...
		return wirtualAPI.CreateInMemoryProvisionerDaemon(dialCtx, name, provisionerTypes)
	}, &provisionerd.Options{
		Logger:              provisionerLogger,
		UpdateInterval:      time.Second,
		ForceCancelInterval: cfg.Provisioner.ForceCancelInterval.Value(),
		Connector:           connector,
		TracerProvider:      wirtualAPI.TracerProvider,
		Metrics:             &metrics,
	}), nil
}
...
func PrintLogo(inv *serpent.Invocation, daemonTitle string) {
	...
	_, _ = fmt.Fprintf(inv.Stdout, "%s - Training Ground For Next Billion Physical AI Agents.\n", versionString)
}
...
func embeddedPostgresURL(cfg config.Root) (string, error) {
	...
	return fmt.Sprintf("postgres://wirtual@localhost:%s/wirtual?sslmode=disable&password=%s", pgPort, pgPassword), nil
}
