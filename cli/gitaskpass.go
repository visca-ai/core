...
// gitAskpass is used by the Wirtual agent to automatically authenticate
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			if err != nil {
				...
				if errors.As(err, &apiError) && apiError.StatusCode() == http.StatusNotFound {
					// This prevents the "Run 'wirtual --help' for usage"
					// message from occurring.
					lines := []string{apiError.Message}
					if apiError.Detail != "" {
						lines = append(lines, apiError.Detail)
					}
					cliui.Warn(inv.Stderr, "Wirtual was unable to handle this git request. The default git behavior will be used instead.",
						lines...,
					)
