...
import (
	...
	"github.com/wirtualdev/wirtualdev/v2/support"
...
func (r *RootCmd) support() *serpent.Command {
	supportCmd := &serpent.Command{
		Use:   "support",
		Short: "Commands for troubleshooting issues with a Wirtual deployment.",
...
var supportBundleBlurb = cliui.Bold("This will collect the following information:\n") +
	`  - Wirtual deployment version
  - Wirtual deployment Configuration (sanitized), including enabled experiments
  - Wirtual deployment health snapshot
  - Wirtual deployment Network troubleshooting information
...
func (r *RootCmd) supportBundle() *serpent.Command {
	...
	var wirtualURLOverride string
	client := new(wirtualsdk.Client)
	cmd := &serpent.Command{
		Use:   "bundle <workspace> [<agent>]",
		Short: "Generate a support bundle to troubleshoot issues connecting to a workspace.",
		Long:  `This command generates a file containing detailed troubleshooting information about the Wirtual deployment and workspace connections. You must specify a single workspace (and optionally an agent name).`,
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			if val, found := os.LookupEnv("WIRTUAL"); found && val == "true" {
				cliui.Warn(inv.Stderr, "Running inside Wirtual workspace; this can affect results!")
				cliLog.Debug(inv.Context(), "running inside wirtual workspace")
			}

			if wirtualURLOverride != "" && wirtualURLOverride != client.URL.String() {
				u, err := url.Parse(wirtualURLOverride)
				if err != nil {
					return xerrors.Errorf("invalid value for Wirtual URL override: %w", err)
				}
				_, _ = fmt.Fprintf(inv.Stderr, "Overrode Wirtual URL to %q; this can affect results!\n", wirtualURLOverride)
				cliLog.Debug(inv.Context(), "wirtual url overridden", slog.F("url", wirtualURLOverride))
				client.URL = u
			}
			...
			if outputPath == "" {
				...
				fname := fmt.Sprintf("wirtual-support-%d.zip", time.Now().Unix())
	...
	cmd.Options = serpent.OptionSet{
		cliui.SkipPromptOption(),
		{
			...
			Description:   "File path for writing the generated support bundle. Defaults to wirtualal-support-$(date +%s).zip.",
			Value:         serpent.StringOf(&outputPath),
		},
		{
			Flag:        "url-override",
			Env:         "WIRTUAL_SUPPORT_BUNDLE_URL_OVERRIDE",
			Description: "Override the URL to your Wirtual deployment. This may be useful, for example, if you need to troubleshoot a specific Wirtual replica.",
			Value:       serpent.StringOf(&wirtualURLOverride),
		},
	}
