...
func TestWorkspaceAgent(t *testing.T) {
	...
	t.Run("LogDirectory", func(t *testing.T) {
		...
		require.Eventually(t, func() bool {
			info, err := os.Stat(filepath.Join(logDir, "wirtual-agent.log"))
			if err != nil {
				return false
			}
			return info.Size() > 0
	...
	t.Run("Headers&DERPHeaders", func(t *testing.T) {
		...
		r := dbfake.WorkspaceBuild(t, wirtualAPI.Database, database.WorkspaceTable{
		...
		agentInv, _ := clitest.New(t,
			...
			"--agent-header-command", "printf X-Process-Testing=very-wow-"+wirtualURLEnv+"'\\r\\n'X-Process-Testing2=more-wow",
		)
