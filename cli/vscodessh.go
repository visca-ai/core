...
// vscodeSSH is used by the Wirtual VS Code extension to establish
...
func (r *RootCmd) vscodeSSH() *serpent.Command {
	...
	cmd := &serpent.Command{
		...
		// passes %h to ProxyCommand. The prefix of `wirtual-vscode--`
		// is a magical string represented in our VS Code extension.
		// It's not important here, only the delimiter `--` is.
		Use:        "vscodessh <wirtual-vscode--<owner>--<workspace>--<agent?>>",
		Hidden:     true,
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			if len(parts) < 3 {
				return xerrors.Errorf("invalid argument format. must be: wirtual-vscode--<owner>--<name>--<agent?>")
			}
			owner := parts[1]
			name := parts[2]
			if len(parts) > 3 {
	...
	cmd.Options = serpent.OptionSet{
		...
		{
			Flag:        "url-file",
			Description: "Specifies a file that contains the Wirtual URL.",
