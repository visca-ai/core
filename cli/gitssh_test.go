...
func TestGitSSH(t *testing.T) {
	...
	t.Run("Local SSH Keys", func(t *testing.T) {
		...
		client, token, wirtualPubkey := prepareTestGitSSH(ctx, t)
		...
		// With the local file deleted, the wirtual key should be used.
		inv, _ = clitest.New(t, cmdArgs...)
		inv.Stdout = pty.Output()
		inv.Stderr = pty.Output()
		err = inv.WithContext(ctx).Run()
		...
		select {
		case key := <-authkey:
			require.Equal(t, wirtualPubkey, key)
		case <-ctx.Done():
			t.Fatal("timeout waiting for auth")
		}
	})
