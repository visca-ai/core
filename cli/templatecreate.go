...
func (r *RootCmd) templateCreate() *serpent.Command {
	...
	cmd := &serpent.Command{
		...
		Middleware: serpent.Chain(
			...
			cliui.DeprecationWarning(
				"Use `wirtual templates push` command for creating and updating templates. \n"+
					"Use `wirtual templates edit` command for editing template settings. ",
			),
			r.InitClient(client),
		),
		Handler: func(inv *serpent.Invocation) error {
			...
			_, _ = fmt.Fprintln(inv.Stdout, "  "+pretty.Sprint(cliui.DefaultStyles.Code, fmt.Sprintf("wirtual create --template=%q --org=%q [workspace name]", templateName, template.OrganizationName)))
			_, _ = fmt.Fprintln(inv.Stdout)
	...
	cmd.Options = serpent.OptionSet{
		...
		{
			Flag:        "failure-ttl",
			Description: "Specify a failure TTL for workspaces created from this template. It is the amount of time after a failed \"start\" build before wirtual automatically schedules a \"stop\" build to cleanup.This licensed feature's default is 0h (off). Maps to \"Failure cleanup\"in the UI.",
			Default:     "0h",
		...
		},
		{
			...
			Description: "Requires workspace builds to use the active template version. This setting does not apply to template admins. This is an enterprise-only feature. See https://wirtual.dev/docs/admin/templates/managing-templates#require-automatic-updates-enterprise for more details.",
