...
func (r *RootCmd) loginWithPassword(
	...
	_, _ = fmt.Fprintf(
		inv.Stdout,
		"Welcome to Wirtual, %s! You're authenticated.",
...
func (r *RootCmd) login() *serpent.Command {
	...
	cmd := &serpent.Command{
		Use:        "login [<url>]",
		Short:      "Authenticate with Wirtual deployment",
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			if err != nil {
				return xerrors.Errorf("Failed to check server %q for first user, is the URL correct and is wirtual accessible from your browser? Error - has initial user: %w", serverURL.String(), err)
			}
			...
			// nolint: nestif
			if !hasFirstUser {
				_, _ = fmt.Fprintf(inv.Stdout, Caret+"Your Wirtual deployment hasn't been set up!\n")
				...
				_, _ = fmt.Fprintf(
					...
					pretty.Sprint(cliui.DefaultStyles.Code, "wirtual templates init"),
				)
				return nil
			}
			...
			_, _ = fmt.Fprintf(inv.Stdout, Caret+"Welcome to Wirtual, %s! You're authenticated.\n", pretty.Sprint(cliui.DefaultStyles.Keyword, resp.Username))
			return nil
		},
	}
	cmd.Options = serpent.OptionSet{
		...
		},
		{
			...
			Description: "Specifies whether a trial license should be provisioned for the Wirtual deployment or not.",
...
// isWSL determines if wirtual-cli is running within Windows Subsystem for Linux
