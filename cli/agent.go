...
import (
	...
	"github.com/wirtualdev/wirtualdev/v2/agent/agentproc"
	"github.com/wirtualdev/wirtualdev/v2/agent/agentssh"
	"github.com/wirtualdev/wirtualdev/v2/agent/reaper"
...
func (r *RootCmd) workspaceAgent() *serpent.Command {
	...
	cmd := &serpent.Command{
		Use:   "agent",
		Short: `Starts the Wirtual workspace agent.`,
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			if reaper.IsInitProcess() && !noReap && isLinux {
				logWriter := &clilog.LumberjackWriteCloseFixer{Writer: &lumberjack.Logger{
					Filename: filepath.Join(logDir, "wirtual-agent-init.log"),
					MaxSize:  5, // MB
			...
			logWriter := &clilog.LumberjackWriteCloseFixer{Writer: &lumberjack.Logger{
				Filename: filepath.Join(logDir, "wirtual-agent.log"),
				MaxSize:  5, // MB
