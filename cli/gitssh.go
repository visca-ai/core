...
func (r *RootCmd) gitssh() *serpent.Command {
	cmd := &serpent.Command{
		Use:    "gitssh",
		Hidden: true,
		Short:  `Wraps the "ssh" command and uses the wirtual gitssh key for authentication`,
		Handler: func(inv *serpent.Invocation) error {
			...
			privateKeyFile, err := os.CreateTemp("", "wirtual-gitsshkey-*")
			if err != nil {
				...
				if xerrors.As(err, &exitErr) && exitErr.ExitCode() == 255 {
					_, _ = fmt.Fprintln(inv.Stderr,
						"\n"+pretty.Sprintf(
							...
							"Wirtual authenticates with "+pretty.Sprint(cliui.DefaultStyles.Field, "git")+
...
// Example invocation:
//
//	ssh -G -o SendEnv=GIT_PROTOCOL git@github.com git-upload-pack 'wirtual/wirtual'
//
// The extra arguments work without issue and lets us run the command
// as-is without stripping out the excess (git-upload-pack 'wirtual/wirtual').
